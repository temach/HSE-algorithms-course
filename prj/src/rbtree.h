...
public:
    ...
    class Node {
        ...
        Node(const Element& key = Element(),
                ...
                Node* parent = nullptr,
                Color col = BLACK)
            : _key(key), _color(col), _left(left), _right(right), _parent(parent)
        {
...
public:
    ...
    /* -----------------------------------------------------------------------------*/
    /**
     * @brief Helper function for insert function.
     *
     * @param z Node to be inserted via Bsl insert.
     */
    ...
    bool isEmpty() const { return _root_parent->_left == _nil; }

    /** \brief Возвращает неизменяемый указатель на корневой элемент. */
    const Node* getRoot() const { return _root_parent->_left;  }
public:
    // Отладочные операции
