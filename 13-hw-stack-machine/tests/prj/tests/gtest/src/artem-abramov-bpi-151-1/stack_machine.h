...
/* -----------------------------------------------------------------------------*/
/**
 *  @defgroup new_operations Classes implementing operations ^ and & 
 *  Created by Артем Абрамов БПИ 151
 *  @{
 */

/**
 * An example of operation object that implements binary AND (x&y) operation
 */
...
/**
 * An example of operation object that implements raise to the POWER (x^y) binary operation
 */
class PowerOp : public IOperation {
    public:
        /** If a given operator symb is not "&", an exception is thrown*/
        virtual int operation(char op, int a, int b, int c) override;
        virtual Arity getArity() const override;
...
/**
 * An example of operation object that implements multiplication (x*y) binary operation
 */
